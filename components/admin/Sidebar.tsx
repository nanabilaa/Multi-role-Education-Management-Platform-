...
const navItems = [
  ...
  { href: '/admin/dana', label: 'Keuangan', icon: DollarSign },
...
export default function Sidebar() {
  ...
  const SidebarContent = () => (
    <aside className="flex h-full flex-col bg-[#063D27] text-white">
      <div className="px-3 pb-3 pt-4">
        <Link
          href="/admin"
          onClick={closeMobile}
          className="group flex items-center gap-3 rounded-[20px] border border-white/10 bg-white/10 p-2.5 transition hover:bg-white/15"
        >
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[15px] bg-[#FFD24D] text-[#063D27] shadow-sm">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {active && <ChevronRight className="h-3.5 w-3.5 text-[#063D27]/70" />}
              </Link>
            )
          })}
        </nav>
