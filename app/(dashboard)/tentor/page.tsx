...
import { Plus, BookOpen, CheckCircle, Calendar, DollarSign } from 'lucide-react'
...
async function getTentorDashboard(userId: string) {
  ...
  const startBulan = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}-01`

  const [sesiHariIni, sesiMingguIni, allSesi] = await Promise.all([
    supabase.from('sesi')
    ...
    supabase.from('sesi')
      ...
      .eq('tentor_id', userId)
      .gte('tanggal', format(startOfWeek, 'yyyy-MM-dd'))
      .lte('tanggal', format(endOfWeek, 'yyyy-MM-dd')),
    supabase.from('sesi')
      .select('durasi, status, tanggal')
      .eq('tentor_id', userId)
      .eq('status', 'selesai')
      .gte('tanggal', startBulan),
  ])
  ...
  return (
    <>
      <style>{`
        ...
        .t-sc-icon { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; margin-bottom: 12px; }
        ...
        .t-card-title { font-size: 13px; font-weight: 600; color: #0f172a; margin-bottom: 14px; display: flex; align-items: center; justify-content: space-between; }
        .t-card-link { font-size: 11px; color: #2557d6; text-decoration: none; font-weight: 500; }
        .t-sesi-item { display: flex; align-items: flex-start; gap: 12px; padding: 12px; border-radius: 12px; border: 1px solid #f1f5f9; margin-bottom: 8px; transition: border 0.15s, background 0.15s; }
        .t-sesi-item:hover { border-color: #bfdbfe; background: #f0f7ff; }
        .t-sesi-item:last-child { margin-bottom: 0; }
        .t-sesi-time { background: #eff6ff; border-radius: 10px; padding: 8px 10px; text-align: center; flex-shrink: 0; min-width: 52px; }
        ...
        .t-btn-masuk { background: #1a3a8f; color: #fff; border: none; border-radius: 8px; padding: 6px 12px; font-size: 11px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; transition: background 0.15s; }
        .t-btn-masuk:hover { background: #2557d6; }
        .t-btn-jurnal { background: #f0f7ff; color: #1a3a8f; border: 1px solid #bfdbfe; border-radius: 8px; padding: 6px 12px; font-size: 11px; font-weight: 500; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; }
        .t-jadwal-item { display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid #f8fafc; }
        .t-jadwal-item:last-child { border-bottom: none; }
        .t-jadwal-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
        ...
        @media (max-width: 640px) {
          .t-content { padding: 14px; gap: 12px; }
          ...
          <div className="t-stat-grid">
            <div className="t-sc feat">
              ...
              <p className="t-sc-sub" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {data.sesiHariIni.filter((s: any) => s.status === 'selesai').length} sudah selesai
              </p>
                  ...
                  <div key={sesi.id} className="t-jadwal-item">
                    ...
                    <span className={`badge badge-${sesi.status}`}>
                      {sesi.status === 'selesai' ? 'Selesai'
                        : sesi.status === 'berlangsung' ? 'Berlangsung' : 'Terjadwal'}
                    </span>
                  </div>
                ))
              )}
